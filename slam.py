...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # NOTE: They call N "time steps", but I really think it should be
    #       called "num_poses", since data hase N-1 elements in it. I would
    #       think that if you have 5 time steps, you have 5 measurements,
    #       and 6 poses (initial plus the 5 after each time step).

    dim = 2 # i.e.: x and y
    matrix_size = (N + num_landmarks) * dim

    movement_strength = 1.0 / float(motion_noise)
    measurement_strength = 1.0 / float(measurement_noise)

    omega = matrix()
    omega.zero(matrix_size, matrix_size)
    xi    = matrix()
    xi.zero(matrix_size, 1)

    # Initial position constraints - assume center of world
    omega.value[0][0] = 1.0
    omega.value[1][1] = 1.0
    xi.value[0][0]    = float(world_size) / 2.0
    xi.value[1][0]    = float(world_size) / 2.0

    for k in range(len(data)):
        Pxi = k * dim         # omega index for pose k's x-coord
        Pyi = Pxi + 1         # omega index for pose k's y-coord
        Qxi  = (k + 1) * dim  # omega index for pose k+1's x-coord
        Qyi  = Qxi + 1        # omega index for post k+1's y-coord

        # Update for all the measurements at this pose
        measurements = data[k][0]
        for z in measurements:
            Lxi = (N + z[0]) * dim # omega index for location's x-coord
            Lyi = Lxi + 1          # omega index for location's y-coord
            Ldx = float(z[1])
            Ldy = float(z[2])
            omega.value[Pxi][Pxi] += measurement_strength
            omega.value[Lxi][Lxi] += measurement_strength
            omega.value[Pxi][Lxi] -= measurement_strength
            omega.value[Lxi][Pxi] -= measurement_strength
            xi.value[Pxi][0]      -= (Ldx * measurement_strength)
            xi.value[Lxi][0]      += (Ldx * measurement_strength)
            
            omega.value[Pyi][Pyi] += measurement_strength
            omega.value[Lyi][Lyi] += measurement_strength
            omega.value[Pyi][Lyi] -= measurement_strength
            omega.value[Lyi][Pyi] -= measurement_strength
            xi.value[Pyi][0]      -= (Ldy * measurement_strength)
            xi.value[Lyi][0]      += (Ldy * measurement_strength)
        ...
        omega.value[Pxi][Pxi] += movement_strength
        omega.value[Qxi][Qxi] += movement_strength
        omega.value[Pxi][Qxi] -= movement_strength
        omega.value[Qxi][Pxi] -= movement_strength
        xi.value[Pxi][0]      -= (Pdx * movement_strength)
        xi.value[Qxi][0]      += (Pdx * movement_strength)

        omega.value[Pyi][Pyi] += movement_strength
        omega.value[Qyi][Qyi] += movement_strength
        omega.value[Pyi][Qyi] -= movement_strength
        omega.value[Qyi][Pyi] -= movement_strength
        xi.value[Pyi][0]      -= (Pdy * movement_strength)
        xi.value[Qyi][0]      += (Pdy * movement_strength)
